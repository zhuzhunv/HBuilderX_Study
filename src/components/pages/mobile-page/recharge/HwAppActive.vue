    ...
    methods: {
        ...
        SeletedParentProduct(item, index) {
            ...
            for (let i = 0; i < item.children.length; i++) {
                if (item.children[i].flowType == '1') {
                    lau = item.children[i]
                    break
                }
            }
            this.$refs.childrenlist.ChangInputData(lau?lau:item.children[0], item.name == '特惠话费' ? item.name + '慢充' : item.name)
        ...
        GoBuy(item) {
            ...
            // console.log(platUid);
            // console.log(platUid, process.env.NODE_ENV);
            if (!platUid) {
            ...
            if (res.code == 100) {
                if (res.data && res.data.length > 0) {
                    ...
                    if (this.getProductData.productType == ConstEnum.ProductTypeEnum.Fee) {
                        ...
                        this.parentListObj.list = ProductSer.FastFlowManagerPush(res.data, 1024)
