<template>
  <div class="HwApp">
    <div>
      ...
      <parent-product-box ref="parternProduct" :theme='theme' :listObj="parentListObj" @seletedItem="SeletedParentProduct">
        <template slot-scope="item">
          <div class="width100" v-if="item.row.productType==productType.fee">
            <template v-if="item.row.key=='vp'">
              <p>
                <span class="value_span">语音包</span>
              </p>
            </template>
            <template v-else>
              <p v-if="item.row.mergeType !='0'" class="fs0">
                <span class="value_span">{{item.row.key}}</span>
                <span class="size28">{{item.row.unit}}</span>
              </p>
              <p v-else>
                <span class="size28 bold">{{item.row.key}}</span>
              </p>
              <p class="size28" v-if="item.row.mergeType !='0'">售价 {{item.row.actualPrice}} 元</p>
              <p class="size24 bold" v-else>{{item.row.fee}}</p>
            </template>
          </div>
          <div class="width100" v-else-if="item.row.productType==productType.flow">
            <p class="fs0">
  ...
  methods: {
    ...
    SeletedParentProduct(item, index) {
      ...
      if (item.mergeType == '0') {
        ...
        this.$refs.childrenlist.ChangInputData(item.children[0], item.name)
